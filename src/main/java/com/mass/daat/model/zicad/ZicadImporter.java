...
public class ZicadImporter {
    ...
    Result performImport()
        throws Exception
    {
        log.atInfo().log("Starting ZICAD import from {}", source);
        Instant start = now();
        ...
        log.atInfo().log("Parsed {} ZICAD zones from {} in {}s", zones.size(), source, Duration.between(start, now()).toMillis() / 1000d);
